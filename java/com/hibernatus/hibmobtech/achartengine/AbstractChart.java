...
import com.hibernatus.hibmobtech.utils.MathHelper;
...
/**
 * An abstract class to be implemented by the  rendering classes.
 */
public abstract class AbstractChart implements Serializable {
    public static final String TAG = AbstractChart.class.getSimpleName();
    ...
    protected void drawString(Canvas canvas, String text, float x, float y, Paint paint) {
        Log.d(TAG, "drawString : text=" + text + " x=" + x + " y=" + y);
    ...
    /**
     * Checks if the current  is rendered as vertical.
     *
     * @param renderer the renderer
     * @return if the  is rendered as a vertical one
     */
     ...
     * @param centerX the round  center on X axis
     * @param centerY the round  center on Y axis
     * @param shortRadius the short radius for the round
     * @param longRadius the long radius for the round
    ...
    protected void myDrawLabel(Canvas canvas, String labelText, DefaultRenderer renderer, int centerX, int centerY, float radius,
                               float currentAngle, float angle, int color, Paint paint) {
        Log.d(TAG, "myDrawLabel : labelText=" + labelText + " centerX=" + centerX + " centerY=" + centerY);

        String st = labelText;
        st = st.replaceAll("\\s+","");
        Log.d(TAG, "myDrawLabel : st=" + st);
        if(st.equals("0")) return; // suppression du label 0

        if (renderer.isShowLabels()) {
            paint.setColor(color);
            float dAngle = 90 - (currentAngle + angle / 2);
            Log.d(TAG, "myDrawLabel : angle=" + angle + " currentAngle=" + currentAngle + " dAngle=" + dAngle);
            int x; // coord x du début du label
            int y; // coord y du début du label
            if(angle == 360.0 && currentAngle == 0.0) { // cas où il n'y a qu'un seul segment
                x = centerX; // on centre le label au milieu du cluster
                y = centerY;
            }
            else{
                double rAngle = Math.toRadians(90 - (currentAngle + angle / 2));
                double sinValue = Math.sin(rAngle);
                double cosValue = Math.cos(rAngle);
                x = Math.round(centerX + (float) (radius * sinValue) / 2);
                y = Math.round(centerY + (float) (radius * cosValue) / 2);
            }

            // calcul de la taille de la string
            paint.setTextAlign(Align.CENTER);
            paint.setTypeface(renderer.getTextTypeface());
            Rect stringSize = new Rect();
            paint.getTextBounds(st, 0, st.length(), stringSize);
            Log.d(TAG, "myDrawLabel : stringSize.width()=" + stringSize.width() + " stringSize.height()=" + stringSize.height());

            // -10 correspond à la largeur de la police de caratère
            canvas.drawText(labelText, x - 8 + (stringSize.width()) / 2, y + (stringSize.height()) / 2, paint);
        }
    }
     ...
     * element. If there is no  element (line, point, bar, etc) at those
