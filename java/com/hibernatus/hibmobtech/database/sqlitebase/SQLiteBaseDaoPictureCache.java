...
import static com.hibernatus.hibmobtech.database.sqlitebase.SQLiteBaseDaoFactory.TABLE_PICTURE_CACHE;
 ...
 */
public class SQLiteBaseDaoPictureCache {
    public static final String TAG = SQLiteBaseDaoPictureCache.class.getSimpleName();
    ...
    public SQLiteBaseDaoPictureCache(Context context){
    ...
    synchronized public void addMroRequestPicture(Picture picture, Long mroRequestId) {
        ...
        sqLiteDatabase.insert(TABLE_PICTURE_CACHE, null, values);
    ...
    synchronized public Picture getPicture(int id) {
        ...
        Cursor cursor = sqLiteDatabase.query(TABLE_PICTURE_CACHE, allColumns , COL_ID + "=?",
...
/*        Log.d(TAG, "cursorToTPicture: cursor=" + cursor
    ...
    synchronized public List<Picture> getAllTablePictures() {
        ...
        // Select All Query
        String selectQuery = "SELECT * FROM " +  TABLE_PICTURE_CACHE + " ORDER BY " + COL_ID;
    ...
    public Cursor getCursorAllPictures(){
        return sqLiteDatabase.query(TABLE_PICTURE_CACHE, allColumns ,
                null, null, null, null,
                COL_TIME + " asc ");
    }
    ...
    synchronized public int updatePicture(Picture picture) {
        ...
        values.put(COL_TIME, picture.getInfos().getTime());
        values.put(COL_RESEND_NUMBER, picture.getResendNumber());
        return sqLiteDatabase.update(TABLE_PICTURE_CACHE, values, COL_ID + " = ?",
    ...
    synchronized public int deletePicture(Picture picture) {
        ...
        if (sqLiteDatabase.isOpen()){
            return sqLiteDatabase.delete(TABLE_PICTURE_CACHE, COL_ID + " = ?",
                    new String[] { String.valueOf(picture.getId()) });
        }
        else {
            open();
    ...
    synchronized public int deleteAllTablePictures(){
        ...
        if (sqLiteDatabase.isOpen()){
            return sqLiteDatabase.delete(SQLiteBaseDaoFactory.TABLE_PICTURE_CACHE, null, null);
        }
        else {
            open();
    ...
    synchronized public int getPicturesCount() {
        ...
        String countQuery = "SELECT * FROM " + TABLE_PICTURE_CACHE;
