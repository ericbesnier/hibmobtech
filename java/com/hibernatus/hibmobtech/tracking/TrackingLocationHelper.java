...
import com.hibernatus.hibmobtech.utils.AlarmUtils;

import java.util.List;

public class TrackingLocationHelper implements LocationListener {

    public static final String TAG = TrackingLocationHelper.class.getSimpleName();
    ...
    private static final TrackingLocationHelper INSTANCE = new TrackingLocationHelper();
    ...
    public static TrackingLocationHelper get() {
        return INSTANCE;
    }

    public TrackingLocationHelper() {

        // Acquire a reference to the system Location Manager
        locationManager = (LocationManager) HibmobtechApplication.getInstance().getSystemService(Context.LOCATION_SERVICE);
        ...
        HibmobtechApplication.getInstance().registerReceiver(stopLocalizationBroadcastReceiver,
    ...
    /**
     * Register the mroRequestClickPagerPictureListerner with the Location Manager to receive location updates
     */
    @RequiresPermission(anyOf = { Manifest.permission.ACCESS_COARSE_LOCATION, Manifest.permission.ACCESS_FINE_LOCATION})
    public synchronized void startLocalization() {
        if (!started && hasSignificantlyOlderLocation()) {
            ...
            // getCauses all enabled providers
    ...
    private PendingIntent getCancelPendingIntent() {
        ...
        return PendingIntent.getBroadcast(HibmobtechApplication.getInstance(), 0, gpsIntent,
                PendingIntent.FLAG_CANCEL_CURRENT);
    }

    /**
     * Remove the mroRequestClickPagerPictureListerner to receive location updates
     */
    @RequiresPermission(anyOf = { Manifest.permission.ACCESS_COARSE_LOCATION, Manifest.permission.ACCESS_FINE_LOCATION})
